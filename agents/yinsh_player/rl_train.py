...
class myAgent():
    ...
    def __init__(self,_id):
        ...
        weight_flie_r = open('weight.txt','r')
        weight_list = []
        weight = weight_flie_r.readline()
        weight = weight.strip()
        weight = weight.split(" ")
        for num in weight:
            weight_list.append(float(num))
        self.weight = pickle.loads(pickle.dumps(weight_list, -1))
        weight_flie_r.close()
        self.count = 0
        self.startTime = None
    ...
    def SelectAction(self, actions, gameState):
        ...
        if random.uniform(0,1) < 1 - epsi:
            ...
            selected_action = random.choice(actions)
            features = self.CalFeatures(pickle.loads(pickle.dumps(gameState, -1)), selected_action)
            Q_value = np.dot(features, self.weight)
            selected_Q_value = Q_value
        next_state = pickle.loads(pickle.dumps(gameState, -1))
        reward = self.DoAction(next_state, selected_action)
        next_actions = self.GetActions(next_state)  
        next_selected_Q_value = -1000000
        for next_action in next_actions:
            if time.time()-start_time <= THINKTIME:
                    features = self.CalFeatures(pickle.loads(pickle.dumps(next_state, -1)), next_action)
                    next_Q_value = np.dot(features, self.weight)
                    if next_Q_value > next_selected_Q_value:
                        next_selected_Q_value =  next_Q_value
            else:
                print("TIME OUT!!")
                break             
        features = self.CalFeatures(pickle.loads(pickle.dumps(gameState, -1)), selected_action)
        ...
        for i in range(len(self.weight)):
            self.weight[i] += alpha * (reward + gamma * next_selected_Q_value - selected_Q_value) * features[i]
        weight_file = open('weight.txt', 'w')  
        for i in range(len(self.weight)):
            weight_file.write(str(self.weight[i]) + " ")
        weight_file.close()
