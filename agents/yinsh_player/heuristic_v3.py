...
class myAgent():
    def __init__(self, _id):
        ...
        self.heuristic = self.defensive_heuristic
    ...
    def SelectAction(self, actions, game_state):
        ...
        if np.count_nonzero(curr_state.board) == 36:
            self.heuristic = self.offensive_heuristic
        ...
        for action in valid_actions:
            ...
            if is_oppo_score_add:
                ...
                heuristic_val = self.heuristic(succ_state)
                # if time.time() - start_time < THINKTIME and not self.IsOppoCanAddScore(succ_state):
                #     if heuristic_val not in heuristic_val_list_remove_flaw:
                #         pq_remove_flaw_action.put((heuristic_val, action))
        ...
        # if pq_remove_flaw_action.qsize():
        #     return pq_remove_flaw_action.get()[1]
        if pq.qsize():
            return pq.get()[1]
